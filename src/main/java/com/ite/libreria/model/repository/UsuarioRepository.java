...
public interface UsuarioRepository extends JpaRepository<Usuario, String>{
	...
	@Query(value= 
			"select sum(cantidad) from lineas_pedido lp inner join pedidos p on lp.id_pedido=p.id_pedido where p.username=?1",
			nativeQuery = true)
	/*@Query("select sum(cantidad) from LineasPedido lp "
			+ "inner join Pedido p "
			+ "where p.username = ?1")*/
